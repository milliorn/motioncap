...
/// IP cameras.
fn resolve_pinned_camera_index(device: Option<&Path>) -> Result<Option<CameraIndex>> {
...
/// open for capture (metadata/IR-sensor nodes). Requires a real camera
/// device and is not covered by an automated test.
fn auto_detect_camera_index() -> Result<CameraIndex> {
...
/// startup. Requires a real camera device and is not covered by an automated
/// test.
...
#[cfg(test)]
mod tests {
    //! Unit tests for `resolve_pinned_camera_index`. `auto_detect_camera_index`
    //! and `start_camera_capture` require a real camera device and are left
    //! untested here.
    #![allow(
        clippy::unwrap_used,
