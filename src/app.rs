//! Top-level wiring: `run()` and the three long-lived worker loops. See the
//! project guidance file and `docs/adr/` for architecture and
...
use crate::confirmation::{ActiveEventPending, PreTriggerPending};
use crate::detect::Detector;
use crate::event_lifecycle::{
    ActiveEvent, close_event_if_done, finish_event_on_shutdown, seed_and_drain_active_event,
};
use crate::liveness::{FrameLiveness, frame_liveness_advanced, reset_liveness_after_reconnect};
use crate::logging::init_logging;
use crate::motion::MotionGate;
use crate::preview::PreviewWindow;
use crate::reconnect::{DetectionCamera, maybe_reconnect_camera};
use crate::triggering::{
    AudioParams, RECORDING_POLL_INTERVAL, evaluate_active_event, try_start_recording,
};
use crate::writer_drained::WriterDrained;
use crate::{MILLIS_PER_SEC, capture, config, startup};
...
/// until shutdown.
///
/// Called by `main`. Opens the real camera and audio devices and spawns
/// long-lived worker threads, none of which can run without physical
/// hardware, so it's not exercised by an automated test.
///
/// # Errors
///
/// Returns an error if logging setup, a startup dependency check, opening
/// the camera/audio devices, or registering the shutdown handler fails.
///
/// # Panics
///
/// Panics if the detection worker or recording writer thread itself panics.
pub fn run() -> Result<()> {
